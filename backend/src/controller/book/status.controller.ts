...
export class StatusController {
    ...
    async create(req: AuthRequest, res: Response) {
        ...
        try {
            ...
            const result = await adminSession.createStatus(req.body.statusName);
    ...
    async getAll(req: AuthRequest, res: Response) {
        ...
        try {
            const adminSession = <AdminSession> req.session;
            const titles = await adminSession.getAllStatuses();
            res.json(titles);
        } catch (error) {
            res.status(500).json({
    ...
    async getByName(req: AuthRequest, res: Response) {
        ...
        try {
            const adminSession = <AdminSession> req.session;
            const title = await adminSession.findStatus(req.params.statusName);
...
export const statusController: StatusController = new StatusController(manager);